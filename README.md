...
- 针对第xxx个问题，根据"题目背景以及建模思路.md"，请你用Python实现xxx模型的完整实现代码。代码中要包含清晰的函数划分，变量命名应有实际含义，避免过度简化。每一步都要有中文注释，解释这一行代码的目的和逻辑。请特别注意输入与输出的格式设计，保证代码能够复用，而不是一次性的脚本。最后，请补充运行样例，并说明运行结果的含义。注意，如果有API可以调用，请优先调用API
